...
public class AntForest {
    ...
    static class FriendRanking {
        public boolean canCollectEnergy;
        public long canCollectLaterTime;
        public boolean canGrabPacket;
        public boolean canHelpCollect;
        public int collectableBubbleCount;
        public int energySummation;
        public boolean forestUser;
        public int rank;
        public int treeAmount;
        public String userId;
    }

    static class FillUserRobFlag {
        public List<FriendRanking> friendRanking;
    ...
    //查询 可以收集能量
    private static void 可以收集能量(ClassLoader loader, List<String> userIds) {
        YLog.i("可以收集能量，进入方法，userIds.size()=" + userIds.size());
        List<String> userTemp = new ArrayList<>();
        //分页请求一次读取100个
        for (int i = 0; i < userIds.size(); i++) {
            userTemp.add(userIds.get(i));
            if (i == userIds.size() - 1 || userTemp.size() >= 20) {
                try {
                    YLog.i("可以收集能量请求：" + userTemp.size());
                    String s = AntForestRpcCall.rpcCall_fillUserRobFlag(loader, userTemp);
                    AntForestNotification.setContentText("今日战绩："+Statistics.getTodayData()+"\n"+"最后一次检测时间："+Log.getFormatTime() );
                    YLog.i("可以收集能量，返回结果：" + s);
                    JSONObject jo = new JSONObject(s);
                    if (jo.getString("resultCode").equals("SUCCESS")) {
                        FillUserRobFlag fillUserRobFlag = JSON.parseObject(s, FillUserRobFlag.class);
                        for (FriendRanking item : fillUserRobFlag.friendRanking) {
                            //找出能偷，能帮，或者等待时间大于0的id，且不是自己
                            boolean optBoolean = (item.canCollectEnergy || item.canHelpCollect || item.canCollectLaterTime > 0);
                            if (optBoolean && !item.userId.equals(selfId)) {
                                userIds.add(item.userId);
                                canCollectEnergy(loader, item.userId, false);
                            } else {
                                FriendIdMap.getNameById(item.userId);
                            }
                        }
                    } else {
                        YLog.d(jo.getString("resultDesc"), s);
                    }
                } catch (Throwable t) {
                    YLog.e("获取可收集能量 错误", t);
                }finally {
    ...
    private static void canCollectSelfEnergy(ClassLoader loader, int times) {
        ...
        try {
            ...
            String s = AntForestRpcCall.rpcCall_queryFriendHomePage(loader, FriendIdMap.currentUid);
            long end = System.currentTimeMillis();
            if (s == null) {
                Thread.sleep(RandomUtils.delay());
                start = System.currentTimeMillis();
                s = AntForestRpcCall.rpcCall_queryFriendHomePage(loader, FriendIdMap.currentUid);
        ...
        try {
            ...
            if (jo.getString("resultCode").equals("SUCCESS")) {
                ...
                for (int i = 0; i < jaBubbles.length(); i++) {
                    ...
                    switch (CollectStatus.valueOf(jo.getString("collectStatus"))) {
                        case AVAILABLE:
                            if (Config.getDontCollectList().contains(userId) || ("2088802524909111".equals(userId)))
                ...
                public void run() {
                    try {
                        ...
                        for (int i = 0; i < unknownIds.length; i++) {
                            long start = System.currentTimeMillis();
                            String s = AntForestRpcCall.rpcCall_queryFriendHomePage(loader, unknownIds[i]);
                            long end = System.currentTimeMillis();
                            JSONObject jo = new JSONObject(s);
                            if (jo.getString("resultCode").equals("SUCCESS")) {
                                serverTime = jo.getLong("now");
